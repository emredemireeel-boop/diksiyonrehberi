...
for (const mod of navItems) {
  // Inject Desktop
  // Find the exact line with data-page="mod" in <a class="nav-link" ...>
  const modIdx = html.indexOf(\`data-page="\${mod}"\`);
  if (modIdx > -1) {
    // We want to insert just before the CLOSING </div> of <div class="mega-menu"> for THIS module.
    // Let's find <div class="mega-menu"> after modIdx
    const menuIdx = html.indexOf('class="mega-menu"', modIdx);
    if (menuIdx > -1) {
      // Find the closing </div> of this mega-menu. It's followed by another </div> for nav-item
      // A quick hack: find the first sequence of spaces + </div>\\n + spaces + </div> after menuIdx.
      // Or simply find "</div>\\n            </div>\\n          </div>" if prettier formatted it.
      // Actually, since we know we want to append it as the LAST mega-col, we can find the LAST </div> inside the mega-menu block.
      // Let's just find "            </div>\\n          </div>" and prepend to it.
      // Because we don't want to mess up, let's use string manipulation carefully.
      
      let endIdx = html.indexOf('</div>\\n          </div>', menuIdx);
      if (endIdx === -1) endIdx = html.indexOf('</div>\\r\\n          </div>', menuIdx);
      
      if (endIdx > -1 && !html.substring(menuIdx, endIdx).includes('Simülasyonlar')) {
...
for (const mod of navItems) {
  ...
  const summaryIdx = html.indexOf(\`<summary class="mob-summary">\${summaryName}</summary>\`);
  if (summaryIdx > -1) {
    let endIdx = html.indexOf('</div>\\n        </details>', summaryIdx);
    if (endIdx === -1) endIdx = html.indexOf('</div>\\r\\n        </details>', summaryIdx);
    
    if (endIdx > -1 && !html.substring(summaryIdx, endIdx).includes('🎮')) {
      html = html.substring(0, endIdx) + "\\n" + appsMob[mod] + "\\n          " + html.substring(endIdx);
    }
  }
}
